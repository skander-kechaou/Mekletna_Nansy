...
if(isset($_GET['mailClient']) && isset($_GET['codeClient'])){
    $_SESSION['mailClient'] = $_GET['mailClient'];
    $code = $_GET['codeClient'];
    // Check against the database to see if correct link
    $query = $db->prepare("SELECT * FROM reset WHERE mailClient = ?");
    $query->execute([$_SESSION['mailClient']]);
...
if(isset($_POST['reset'])){
    ...
    // Update password
    if(empty($error)){
        $query = $db->prepare("UPDATE Client SET pwdClient = ? WHERE mailClient = ?");
